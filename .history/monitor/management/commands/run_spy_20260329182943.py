...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for task in tasks:
            ...
            if not items:
                self.stdout.write(self.style.WARNING(f"⚠️ 爬虫未抓取到任何数据，请检查网络或 IP 是否被封。"))
            else:
                self.stdout.write(self.style.SUCCESS(f"🚀 爬虫抓取回了 {len(items)} 条数据，开始匹配关键词..."))
            print(f"DEBUG: 爬虫抓回了 {len(items)} 个原始数据")
            new_items_found = []
            ...
            for item in items:
                ...
                if task.must_contain:
                    ...
                    # 进阶逻辑：同时检查“带空格”和“不带空格”的情况
                    title_no_spaces = title_lower.replace(" ", "")
                    has_required_info = any(word in title_lower or word.replace(" ", "") in title_no_spaces for word in required_words)
