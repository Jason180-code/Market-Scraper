...
class Command(BaseCommand):
    ...
    def send_summary_email(self, task, items):
        """封装邮件发送逻辑"""
        prefix = "[VIP 极速提醒]" if getattr(task, 'is_vip', False) else "[每日捡漏日报]"
        subject = f"{prefix} 发现 {len(items)} 件新商品！"
        
        content_lines = [
            f"你好 {task.user.username}，",
            f"\n这是针对关键词「{task.keywords}」的最新监控结果：",
            "\n" + "-"*30
        ]
        for i, item in enumerate(items, 1):
            content_lines.append(f"{i}. 【${item['price']}】{item['title']}")
            content_lines.append(f"   直达链接: {item['link']}\n")
