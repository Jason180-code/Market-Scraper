...
class Command(BaseCommand):
    ...
    def send_combined_summary(self, task, items):
        """发送多平台汇总邮件"""
        # 按价格升序排序
        items.sort(key=lambda x: x.price if hasattr(x, 'price') else x.get('price', 0))
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"  ❌ 邮件发送失败: {e}"))def send_combined_summary(self, task, items):
    """发送多平台汇总邮件"""
