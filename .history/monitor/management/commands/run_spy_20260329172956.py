...
class Command(BaseCommand):
    ...
    'tempered', 'film', 'lens', 'dummy', 'box only', 'parts']
            ...
            for item in items:
                ...
                # --- ✨ 2. 动态特征 & 价格下限校验 (新添加的部分在这里) ---
                
                # A. 动态价格过滤：过滤掉低价配件
                if price < float(task.min_price):
                    # print(f"⏩ 价格低于该任务设定的下限 (${task.min_price})，跳过。")
                    continue

                # B. 动态特征校验：确保包含真机关键词（如 GB, Unlocked）
                if task.must_contain:
                    required_words = [w.strip().lower() for w in task.must_contain.split(',')]
                    has_required_info = any(word in title_lower for word in required_words)
                    
                    if not has_required_info:
                        # print(f"⏩ 标题未包含任务要求的特征词: {task.must_contain}")
                        continue
                ...
                if not ScrapedItem.objects.filter(url=item['link']).exists():
                    try:
                        ...
                        print(f"✨ 发现真机好货: {title[:40]}... ${i['price']}")
