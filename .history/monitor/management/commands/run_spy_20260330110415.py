        ...
        for task in tasks:
           # 动态获取当前任务的用户
            ...
            if newly_discovered and task.user.email:
                ...
                subject = f"ebay自动监控通知：发现 {len(newly_discovered)} 件新商品！"
                
                content_lines = [f"你好 {task.user.username}，本次扫描为您发现以下符合需求的内容：\n"]
                for i, idx_item in enumerate(newly_discovered, 1):
                    content_lines.append(f"{i}. 【价格：${idx_item['price']}】{idx_item['title']}")
